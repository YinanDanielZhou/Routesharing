...
The car sends the x coordinate of each point to one set of servers
The car sends the y coordinate of each point to another set of severs
the two sets are mutually exclusive

For each server, it either has only x coordinates, or only y coordinates
So when a server is compromised, it will only expose a subset of one coordinates
...
def calculate_similarity_score(car_id, total_server, compromised_server_list, num_of_samples, should_plot = False):
    ...
    # since each server now contains only x or only y, it will contain twice amount of x (or y)
    #   compare to scenarios when x and y are allowed to mix in one server
    # so the compromised_sample_number need to double.
    ...
    # non of the y coordinates are compromised, so the attacker takes a best guess
    # we simulate this best guess by setting the y coordinate as the average of all y coordinates.
    y_average = np.mean(y_reduced)
